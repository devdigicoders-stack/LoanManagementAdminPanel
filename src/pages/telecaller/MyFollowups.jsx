          ...
          if (l.followUps && l.followUps.length > 0) {
            l.followUps.forEach(fu => {
              extracted.push({
                ...
                customer: l.name,
                mobile: l.mobile,
                date: fu.scheduledAt || 'Today',
                time: "11:00 AM",
                type: fu.type || 'Call',
                purpose: fu.notes || l.loanPurpose,
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr style={{ background: tc.sky }}>
                {["Lead ID", "Customer Name", "Contact", "Date", "Type", "Discussion Focus / Notes", "Status", "Actions"].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                followups.map((fu, i) => {
                  return (
                    <tr key={fu.followupId || i} style={{ borderBottom: `1px solid ${tc.border}` }} className="hover:bg-[#FAFCFD] transition-colors">
                      ...
                      <td className="px-4 py-3" style={{ color: tc.muted }}>{fu.date}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <Link 
                            to={`/telecaller/calls/${fu.mongoLeadId}`}
                          ...
                          {fu.status !== 'Completed' && (
                            <button
                              ...
                              className="px-2 py-1 bg-green-50 hover:bg-green-100 text-green-700 font-bold rounded-md text-[11px] transition-colors"
                            >
                              Done
                            </button>
                          )}
