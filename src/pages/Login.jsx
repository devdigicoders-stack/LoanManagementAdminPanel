...
const LoginPage = () => {
 ...
 // For Super Admin and Admin, do strict role check
 ...
 if (backendRoleStr !== 'admin' && backendRoleStr !== 'administrator') {
 toast.error(`Invalid role selected. This account is assigned as ${actualRole || 'another role'}.`);
 setIsLoading(false);
 return;
 }
 ...
 navigate('/agent');
 } else if (cleanActual === 'arm') {
 navigate('/sales/arm-dashboard');
 } else if (cleanActual === 'rm' || cleanActual.includes('reportingmanager')) {
 navigate('/sales/rm-dashboard');
 } else if (
 cleanActual === 'saleshead' ||
 cleanActual === 'rrm' ||
 cleanActual === 'ro' ||
 cleanActual === 're' ||
 cleanActual.includes('sales')
 ) {
 ...
 // Check Regional / RRM
 if (raw.includes('regional') || s === 'rrm') return 'rrm';
 // Check Area / ARM
 if (raw.includes('area') || s === 'arm') return 'arm';
 // Check Sales Head
 if (raw.includes('sales head') || s === 'saleshead') return 'saleshead';
 // Check Reporting Manager / RM (Must not match RRM or ARM)
 if (raw === 'reporting manager (rm)' || raw === 'reporting manager' || s === 'rm' || s === 'reportingmanager') return 'rm';
 // Check RO / RE / Relationship Officer / Executive
 if (raw.includes('relationship') || s === 'ro' || s === 're' || s === 'relationshipofficer' || s === 'relationshipexecutive') return 'ro';
 // HR hierarchy
 if (raw.includes('hr head') || s === 'hrhead') return 'hrhead';
 if (raw.includes('hr manager') || s === 'hrmanager') return 'hrmanager';
 if (raw.includes('hr executive') || s === 'hrexecutive' || s === 'hrexec') return 'hrexecutive';
 if (raw === 'hr' || raw.includes('hr admin') || s === 'hradmin') return 'hradmin';
 if (raw.includes('telecaller') || s.includes('telecaller')) return 'telecaller';
 if (raw.includes('agent') || s.includes('agent')) return 'agent';
 if (raw.includes('accountant') || s.includes('accountant')) return 'accountant';
 if (raw.includes('credit') || s.includes('credit')) return 'credit';
 if (raw.includes('operation') || s.includes('operation')) return 'operation';
 if (raw === 'super admin' || s === 'superadmin') return 'superadmin';
 if (raw === 'admin' || s === 'admin') return 'admin';
 return s;
 };
 ...
 // Strict exact role comparison (do not use substring .includes)
 const isExactMatch = backendNorm === uiNorm || desigNorm === uiNorm;
 ...
 if (backendNorm === 'telecaller' || uiNorm === 'telecaller') {
 navigate("/telecaller");
 } else if (backendNorm === 'agent' || uiNorm === 'agent') {
 navigate("/agent");
 } else if (backendNorm === 'arm' || uiNorm === 'arm') {
 navigate("/sales/arm-dashboard");
 } else if (backendNorm === 'rm' || uiNorm === 'rm') {
 navigate("/sales/rm-dashboard");
 } else if (
 backendNorm === 'saleshead' ||
 backendNorm === 'rrm' ||
 backendNorm === 'ro' ||
 backendNorm === 'sales' ||
 uiNorm === 'saleshead' ||
 ...
 <img src="/loanlogo.png" alt="Logo" className="h-[90px] object-contain" />
 </div>
 <h2 className="text-[32px] font-bold text-[var(--color-brand-blue-dark)] tracking-tight leading-snug">
 Welcome to the Loan Management System
 </h2>
 <p className="text-[15px] text-[var(--color-brand-text-secondary)] mt-2 leading-relaxed">
 Manage your loans, applications, and customer relationships efficiently.
 </p>
 </div>

 <div className="grid grid-cols-2 gap-4 max-w-lg">
 <div className="rounded-[14px] p-4 bg-white/70 backdrop-blur-sm border border-[var(--color-brand-border)] flex items-start gap-3 shadow-sm hover:shadow-md transition-shadow">
 <div className="p-2.5 rounded-[10px] bg-[var(--color-brand-sky-light)] text-[var(--color-brand-blue-dark)]">
 <Users size={20} />
 </div>
 <div>
 <h4 className="text-[13px] font-bold text-[var(--color-brand-text)]">Admin Portal</h4>
 ...
 <div>
 <h4 className="text-[13px] font-bold text-[var(--color-brand-text)]">Analytics</h4>
 <p className="text-[11px] text-[var(--color-brand-text-secondary)] leading-relaxed">Track loan performance<br />and metrics</p>
 </div>
 </div>
 </div>
 </div>
